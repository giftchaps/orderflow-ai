...
export default function SettingsPage() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      const { data: biz } = await supabase.from("businesses").select("*").eq("id", staff.business_id).single()
      setBusiness(biz)
    }
    load()
  }, [])
  ...
  const kitchenUrl = business?.slug
    ? `${window.location.origin}/display/${business.slug}`
    : ""
      ...
      <Card className="border-border">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Input type="number" value={business.prep_time_minutes ?? 15} onChange={(e) => setBusiness({ ...business, prep_time_minutes: parseInt(e.target.value) })} min={5} max={60} className="w-32" />
          </div>
      ...
      <Card className="border-border">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center gap-2 mt-2">
            ...
            <span className="text-sm text-muted-foreground">{business.vapi_assistant_id ? "Agent is live" : "Agent not configured"}</span>
          </div>
        </CardContent>
      </Card>
    </div>
