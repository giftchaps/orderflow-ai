...
import { ShoppingBag, TrendingUp, Clock, Radio, ExternalLink, ArrowLeft } from "lucide-react"
import Link from "next/link"

type Props = { params: Promise<{ slug: string }> }

export default async function AdminBusinessDashboardPage({ params }: Props) {
  ...
  const { data: business } = await supabase
    .from("businesses")
    .select("id, name, slug, phone_number, vapi_assistant_id")
    .eq("slug", slug)
    .single()
  ...
  const [
    ...
    supabase.from("orders").select("*", { count: "exact", head: true }).eq("business_id", business.id).gte("placed_at", todayStart),
    supabase.from("orders").select("*", { count: "exact", head: true }).eq("business_id", business.id).gte("placed_at", weekStart),
    supabase.from("orders").select("id, order_number, status, channel, placed_at, items").eq("business_id", business.id).order("placed_at", { ascending: false }).limit(5),
  ])
    ...
    <div className="p-8 space-y-8">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <Link href={`/admin/businesses/${slug}`}>
          ...
          <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">Viewing as admin</p>
          </div>
        </div>
        {kitchenUrl && (
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-border">
                  {(recentOrders ?? []).map((order) => {
                    ...
                    const summary = items.map((i: { qty: number; name: string }) => `${i.qty > 1 ? `${i.qty}x ` : ""}${i.name}`).join(", ")
                    return (
