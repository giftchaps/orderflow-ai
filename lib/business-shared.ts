...
/**
 * Client-safe business types, constants and pure helpers. This module must
 * not import anything server-only — components rendered under a "use client"
 * ancestor (business detail tabs, panels, team manager, settings form) import
 * from here directly. The Supabase-backed read helpers live in lib/business.ts.
 */
...
export function buildSetupChecklist(
  ...
  return items.filter((item) => scope === "admin" || !item.adminOnly)
}
